...
from .stopping import StopController, largest_gap
...
def _run_episode(config: Config) -> Episode:
    ...
    controller = StopController(config.stopping)
    stop_grid = np.r_[candidates, (candidates + TAU / (2 * settings.candidates)) % TAU]
    ...
    means, stds, metrics, model_diagnostics = [], [], [], []
    status = "budget_exhausted"
    current_angle = float(candidates[0])
    total_time, total_distance = 0.0, 0.0

    def record(fit_seconds=0.0, selection_seconds=0.0, innovation=None, attempted=False):
        mean, std = model.predict(angles)
        half_width = float(np.max(1.96 * model.predict(stop_grid)[1]))
        gap = largest_gap([o.theta for o in observations])
        means.append(mean)
        stds.append(std)
        metrics.append(
            {
                ...
                **model.hyperparameters,
                ...
                "max_gap": gap,
                "audits_passed": controller.audits_passed,
                "audit_pending": int(controller.audit_pending),
            }
        )
        model_diagnostics.append(
            {
                ...
                **model.hyperparameters,
                ...
                "optimization_attempted": attempted,
                "message": model.fit_message,
                "warnings": model.fit_warnings,
            }
        )
        return half_width, gap
    ...
    for step in range(settings.touches):
        ...
        elif controller.audit_pending:
            decision = replace(gap_policy.choose(state, model), reason="audit")
        elif (
            settings.safeguard_every
            and (step - settings.initial + 1) % settings.safeguard_every == 0
        ):
            decision = replace(gap_policy.choose(state, model), reason="coverage")
        else:
        ...
        model.fit(tuple(observations), optimize=attempted)
        ...
        half_width, gap = record(fit_seconds, selection_seconds, innovation, attempted)
        if len(observations) >= settings.initial:
            done = controller.observe(
                touches=len(observations),
                max_half_width=half_width,
                max_gap=gap,
                innovation=innovation,
                was_audit=decision.reason == "audit",
            )
            metrics[-1].update(
                audits_passed=controller.audits_passed, audit_pending=int(controller.audit_pending)
            )
            if done:
                status = "confident"
                break
...
def save_episode(result: Episode, folder: str | Path) -> None:
    ...
    write_json(
        ...
        {**provenance(), "status": result.status, "actual_touches": len(result.observations)},
    )
    ...
    with (folder / "model.jsonl").open("w") as handle:
        for item in result.model_diagnostics:
